...
const absensiMahasiswaController = {
    ...
    getRekapAbsensiMahasiswa: async (req, res) => {
        try {
            ...
            // Ambil semua jadwal yang ada untuk praktikum ini
            const allJadwal = await prisma.jadwal.findMany({
                where: { praktikum_id: praktikumId },
                orderBy: { tanggal: 'asc' }
            });

            // Ambil data absensi mahasiswa untuk semua jadwal di praktikum ini
            ...
            // Buat map untuk pencarian status absensi yang cepat
            const absensiMap = new Map(allAbsensi.map(a => [a.jadwal_id, a.status]));

            // Gabungkan data jadwal dengan status absensi mahasiswa
    ...
    exportAbsensiMahasiswaPDF: async (req, res) => {
        try {
            ...
            // 1. Ambil data yang sama seperti di getRekapAbsensi
            ...
            // 2. Inisialisasi dokumen PDFKit
            const doc = new PDFDocument({ size: 'A4', margin: 50 });

            // 3. Atur header respons untuk download
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename=Rekap-Absensi-${user.username}.pdf`);

            // 4. Pipe output PDF langsung ke respons HTTP
            doc.pipe(res);
